...
from inspect_web.handlers.log_utils import FULL_LOG_TOOLS, format_tool_result
...
def before_tool_callback(
    ...
    try:
        ...
        # agent_name calculation removed (unused)
        try:
            # Just check if we can access it to ensure valid context
                    ...
                    for item in content:
                        if isinstance(item, dict):
                            ...
                            if item.get('type') == 'image' and item.get('data'):
                                ...
                                agent_log.debug("🔍 Found base64 screenshot data")
                                break
